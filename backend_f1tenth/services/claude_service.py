"""
Claude AI Service — F1Tenth Vision-Inspect
...
class ClaudeService:
    ...
    def cross_reference(self, visual_analysis: dict, audio_transcription: dict, frames_b64: list, history: list = None) -> dict:
        ...
        Maps the component to the F1Tenth inspection checklist and grades it.
        """
        start = time.time()
        ...
        response = self.client.messages.create(
            ...
            tools=[{
                ...
                "description": "Submit the final cross-reference verdict mapping the inspection to the F1Tenth checklist.",
...
STEP 1 — OBSERVE: Describe exactly what you see in the frames. Note details like tire wear or damage, shock absorber condition, bumper integrity, sensor mounting, cable routing, connector security, and structural integrity.
...
STEP 3 — ASSESS: Evaluate the component's condition. Consider: Is it securely mounted? Are there signs of damage, wear, or loose connections? Could this cause a failure or unsafe behavior during a race run?

STEP 4 — CONCLUDE: State your preliminary assessment. Use one of: PASS (acceptable condition), MONITOR (minor concerns, track next run), FAIL (unsafe, must fix before running), or UNCLEAR (cannot determine from these frames).
...
1. A visual AI analysis of car frames (what the AI SEES)
2. The inspector's spoken assessment (what the inspector SAID)
3. Historical inspection logs (past grade and condition)

Your job is to cross-reference these three sources, map the inspected component to the F1Tenth Pre-Run Checklist below, compare current condition to history, and produce a final graded verdict.
...
STEP 2 — COMPARE: What did the inspector say vs what the AI sees vs History?
  - Do they agree or disagree?
  - Does new visual show degraded condition vs historical baseline?
